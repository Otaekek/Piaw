...
{
	cluster_id = dynamicMemoryManager::cluster_init(sizeof(t_transform), 65536 * 10);
}
...
void	transformBuiltin::reset(uint32_t ref)
{
...
void	transformBuiltin::copy(uint32_t b, uint32_t h)
{
	t_transform *s = get_transform(b);
	t_transform *d = get_transform(h);

	memcpy(s, d, sizeof(t_transform));
}
...
glm::quat transformBuiltin::CreateFromAxisAngle(glm::vec3 axis, float angle)
{
...
glm::quat transformBuiltin::LookAt(glm::vec3 d)
{
	d = glm::normalize(d);
