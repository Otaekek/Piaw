...
void noiseSeed(t_noise * noise, long long unsigned int seed) {
	...
	int i;
	for (i = 0 ; i < 512; i++) {
		swap_from	= noiseNextInt(&seed) & 255;
		swap_to		= noiseNextInt(&seed) & 255;
...
float OctavePerlin(float x, float y, int octaves, float persistence, t_noise *noise) {
	float total = 0;
	float frequency = 1;
	float amplitude = 1;
	float maxValue = 0;
	for(int i=0;i<octaves;i++) {
		total += noise2(noise, x * frequency, y * frequency) * amplitude;
		maxValue += amplitude;
