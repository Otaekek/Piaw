...
private:
	...
	static uint32_t				load_shader(std::string path, int type);
	static uint32_t				load_programTesselation(std::string vertexPath, std::string fragPath, std::string tesPath, std::string tcsPath);
};
