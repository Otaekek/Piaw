...
{
	static const char listext[12][12] = {".jpg", ".TGA", ".tga", ".psd", "\0"};
	...
	for (uint32_t j = 0; j < 2; j++)
	{
		...
		for (int k = strlen(complete_path); complete_path[k] != '/' && k; k--)
			if (complete_path[k] >= 'A' && complete_path[k] <= 'Z') complete_path[k] += 'a' - 'A';
	}
}
...
uint32_t renderDataSys::load_shader(std::string path, int type)
{
	GLchar			*code;
	GLuint			shaderObject;
	GLint			length;
...
uint32_t renderDataSys::load_programTesselation(std::string vertexPath, std::string fragPath, std::string tesPath, std::string tcsPath)
{
	GLint ProgramObject = glCreateProgram();
	glAttachShader(ProgramObject, load_shader(vertexPath, GL_VERTEX_SHADER));
	glAttachShader(ProgramObject, load_shader(fragPath, GL_FRAGMENT_SHADER));
	return 0;
	glAttachShader(ProgramObject, load_shader(tesPath, GL_TESS_EVALUATION_SHADER));
	glAttachShader(ProgramObject, load_shader(tcsPath, GL_TESS_CONTROL_SHADER));
