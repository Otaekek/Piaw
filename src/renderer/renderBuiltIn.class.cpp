...
uint32_t				renderBuiltIn::vox_cluster_id;
...
//	mode->height *= 2;
	...
	GLFWmonitor *  q = glfwGetPrimaryMonitor();
...
uint32_t			renderBuiltIn::createVox()
{
	return dynamicMemoryManager::create_slot(vox_cluster_id);
}

void				renderBuiltIn::destroyVox(uint32_t ref)
{
	dynamicMemoryManager::clear_data(ref, vox_cluster_id);
}
...
void					renderBuiltIn::swap_buffer()
{
	glfwSwapBuffers(window);
}
...
void			renderBuiltIn::render(t_camera *camera)
{
	...
	{
		t_renderGO*	e = (t_renderGO*)renderBuiltIn::get_renderGO(list[i]);
		renderBuiltIn::render_object(e, camera);
...
void						renderBuiltIn::destroy(uint32_t w)
{
	dynamicMemoryManager::clear_data(w, cluster_id);
}
